...
from app.core.config import load_search_settings, save_search_settings, load_llm_parameters, load_settings, save_default_language, save_llm_parameters, load_language_settings

from app.ui.session_callbacks import SessionUIState
from app.ui.preset_callbacks import PresetUIState
...
def clamp_float(value: float | int | str, minimum: float, maximum: float, default: float) -> float:
    try:
        parsed = float(value)
...
def clamp_int(value: float | int | str, minimum: int, maximum: int, default: int) -> int:
    try:
...
def build_demo() -> gr.Blocks:
    ...
    server_choices = [(str(host.get("server_name", "Server")), f"{host.get('address', '')}:{host.get('port', 11434)}") for host in hosts if isinstance(host, dict) and host.get("address")]
